...
class AIClient:
    ...
    async def _complete_with(
        ...
        # Alguns modelos/rotas podem devolver metadados do filtro de segurança
        # em vez da resposta conversacional. Isso deve acionar o fallback.
        safety_markers = ("user safety:", "response safety:", "safety response:")
        if any(marker in text.lower() for marker in safety_markers):
            raise AIProviderError("o modelo retornou metadados de segurança em vez da resposta")
